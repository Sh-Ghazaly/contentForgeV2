...
async function save() {
  ...
  try {
    ...
    const orig = originalSettings.value
    const results = []
    ...
    originalSettings.value = { ...cur }
    saveMsg.value = results.join(' — ')
    saved.value = true
    setTimeout(() => { saved.value = false; saveMsg.value = '' }, 4000)
  } catch (err) {
    saveMsg.value = '✗ ' + (err.response?.data?.message || err.message || 'حدث خطأ')
  } finally {
    saving.value = false
  }
}
