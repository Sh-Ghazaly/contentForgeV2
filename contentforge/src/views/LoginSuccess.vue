  ...
  onMounted(async () => {
    ...
    if (token) {
      try {
        ...
        localStorage.setItem(
          ...
          JSON.stringify({
            ...
            trialExpired:
              userData.isTrial &&
              userData.planEndsAt &&
              new Date() > new Date(userData.planEndsAt),
          }),
        );
        ...
        if (stored) {
          try { pendingRedirect = JSON.parse(stored); } catch (e) { }
          sessionStorage.removeItem('pending_auth_redirect');
        }

        const targetPlan = route.query.plan || pendingRedirect?.plan;
        const targetBilling = route.query.billing || pendingRedirect?.billing;
