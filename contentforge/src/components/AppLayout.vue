<template>
  <div class="min-h-screen theme-bg flex flex-col" :dir="locale === 'ar' ? 'rtl' : 'ltr'">
    ...
    <header class="md:hidden sticky top-0 z-30 h-[57px] px-3 flex items-center theme-glass border-b"
      ...
      <div class="relative ms-auto">
        <button @click="showNotifDropdown = !showNotifDropdown" class="relative p-2">
      ...
      <Transition name="dropdown-fade">
        <div v-if="showNotifDropdown"
          ...
          <div v-for="notif in notifications" :key="notif._id || notif.id"
            class="px-4 py-3 border-b cursor-pointer transition-colors" :class="[!notif.read ? 'bg-blue-500/5' : '']"
            style="border-color: var(--border)" @click="handleNotifClick(notif._id || notif.id)">
            <div class="flex items-start gap-2.5">
              <div class="w-2 h-2 rounded-full mt-1.5 shrink-0" :class="notif.read ? 'bg-transparent' : 'bg-blue-400'">
              </div>
    ...
    <!-- Desktop Navbar -->
    <header class="hidden md:flex theme-glass border-b sticky top-0 z-30 h-[57px] px-4 items-center justify-between"
      style="border-color: var(--border)">
          ...
          <Transition name="dropdown-fade">
            <div v-if="showNotifDropdown"
              ...
              <div class="max-h-80 overflow-y-auto">
                ...
                <div v-for="notif in notifications" :key="notif._id || notif.id"
                  class="px-4 py-3 border-b cursor-pointer transition-colors group relative" :class="[
                    !notif.read ? 'bg-blue-500/5' : '',
                    'hover:bg-blue-500/5',
                  ]" style="border-color: var(--border)" @click="handleNotifClick(notif._id || notif.id)">
                  <div class="flex items-start gap-2.5">
                    <div class="w-2 h-2 rounded-full mt-1.5 shrink-0"
                      :class="notif.read ? 'bg-transparent' : 'bg-blue-400'"></div>
                    <div class="flex-1 min-w-0">
                      <p class="text-xs font-medium theme-text truncate">
                        {{ notif.title }}
                      </p>
                    ...
                    <button @click.stop="deleteNotif(notif._id || notif.id)"
    ...
    <div class="flex flex-1" style="min-height: 0">
      ...
      <aside
        ...
        <nav class="space-y-0.5 flex-1">
          <RouterLink v-for="item in unlockedNavItems" :key="item.path" :to="item.path" @click="sidebarOpen = false"
          ...
          <div v-for="item in lockedNavItems" :key="item.path" @click="!item.comingSoon && $router.push('/payment')"
            ...
            <div class="relative">
              ...
              <svg class="w-2.5 h-2.5 text-amber-400 absolute -bottom-1 -right-1 bg-slate-900 rounded-full p-0.5"
...
const navItems = computed(() => [
...

  {
    ...
    locked: isNotProOrEnterprise.value,
    comingSoon: false
  },
