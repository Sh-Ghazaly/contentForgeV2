<template>
  <section
    id="pricing"
    class="py-24 relative overflow-hidden transition-colors duration-300"
  ...
  >
    ...
    ></div>
    <div
      class="absolute inset-0 bg-grid transition-opacity duration-300"
    ...
    <div class="relative z-10 max-w-6xl mx-auto px-6">
      <div class="text-center mb-16">
        <span
          class="text-xs font-medium uppercase tracking-widest mb-4 block"
          :class="isDark ? 'text-amber-400' : 'text-amber-600'"
        >
          {{ t("pricing.eyebrow") }}
        </span>
        <h2
          class="font-display text-4xl md:text-5xl font-700 mb-4"
          :class="isDark ? 'text-white' : 'text-slate-900'"
        >
          {{ t("pricing.heading") }}<br />
          <span class="text-gradient-warm">{{
        ...
        <div
          class="flex items-center justify-center gap-3 mt-8 flex-wrap select-none"
        >
          <span
      ...
      <div class="grid md:grid-cols-3 gap-5 md:gap-6">
        ...
        >
          ...
          <button
            v-else
            @click="handlePaidPlanClick($event, plan.key)"
...
onMounted(async () => {
  ...
  if (token) {
    try {
      ...
      console.log("User Plan:", userPlan.value, "Billing:", userBilling.value);
    } catch (err) {
...
const plans = computed(() => [
  {
    ...
    tagline: t(
      ...
      "Explore the essential capabilities of our platform"
    ),
    monthlyPrice: 0,
    annualPrice: 0,
    popular: false,
